...
bool mutate_bias_b4 = true;

static bool evaluateCondition(void)
{
   return (mutate_bias_b4);
}

static void evaluatePredicatemutate_bias_4(Morphism *morphism)
{
   ...
   if(var_1 + 1 <= 1000) mutate_bias_b4 = true;
   else mutate_bias_b4 = false;
}

static bool match_n0(Morphism *morphism);

bool matchmutate_bias_bias5(Morphism *morphism)
{
   ...
   for(nodes = getRootNodeList(mutate_bias_host); nodes != NULL; nodes = nodes->next)
   {
      ...
      do
      {
         ...
         if(result >= 0)
         {
            ...
            evaluatePredicatemutate_bias_4(morphism);
...
void applymutate_bias_bias5(Morphism *morphism, bool record_changes)
{
...
bool fillpotmutate_bias_bias5(MorphismPot *pot, Morphism *morphism)
{
   if(1 > mutate_bias_host->number_of_nodes || 0 > mutate_bias_host->number_of_edges) return false;
...
static bool fillpot_n0(MorphismPot *pot, Morphism *morphism)
{
   ...
   for(nodes = getRootNodeList(mutate_bias_host); nodes != NULL; nodes = nodes->next)
   {
      ...
      if(match)
      {
         ...
         host_node->matched = true;
         /* All items matched! */
            putMorphism(pot, morphism, "mutate_bias_bias5", mutate_bias_host);
