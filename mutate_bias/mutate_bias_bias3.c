...
bool mutate_bias_b2 = true;

static bool evaluateCondition(void)
{
   return (mutate_bias_b2);
}

static void evaluatePredicatemutate_bias_2(Morphism *morphism)
{
   ...
   if(var_0 - 1 >= 1000) mutate_bias_b2 = true;
   else mutate_bias_b2 = false;
}

static bool match_n0(Morphism *morphism);

bool matchmutate_bias_bias3(Morphism *morphism)
{
   ...
   for(nodes = getRootNodeList(mutate_bias_host); nodes != NULL; nodes = nodes->next)
   {
      ...
      do
      {
         ...
         if(result >= 0)
         {
            ...
            evaluatePredicatemutate_bias_2(morphism);
...
void applymutate_bias_bias3(Morphism *morphism, bool record_changes)
{
   int var_0 = getIntegerValue(morphism, 0);
...
bool fillpotmutate_bias_bias3(MorphismPot *pot, Morphism *morphism)
{
   if(1 > mutate_bias_host->number_of_nodes || 0 > mutate_bias_host->number_of_edges) return false;
...
static bool fillpot_n0(MorphismPot *pot, Morphism *morphism)
{
   ...
   for(nodes = getRootNodeList(mutate_bias_host); nodes != NULL; nodes = nodes->next)
   {
      ...
      if(match)
      {
         ...
         host_node->matched = true;
         /* All items matched! */
            putMorphism(pot, morphism, "mutate_bias_bias3", mutate_bias_host);
