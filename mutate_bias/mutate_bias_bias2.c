...
bool mutate_bias_b1 = true;

static bool evaluateCondition(void)
{
   return (mutate_bias_b1);
}

static void evaluatePredicatemutate_bias_1(Morphism *morphism)
{
   ...
   if(var_0 + 2 <= 1000) mutate_bias_b1 = true;
   else mutate_bias_b1 = false;
}
   ...
   for(nodes = getRootNodeList(mutate_bias_host); nodes != NULL; nodes = nodes->next)
   {
      ...
      do
      {
         ...
         if(result >= 0)
         {
            ...
            evaluatePredicatemutate_bias_1(morphism);
...
static bool fillpot_n0(MorphismPot *pot, Morphism *morphism)
{
   ...
   for(nodes = getRootNodeList(mutate_bias_host); nodes != NULL; nodes = nodes->next)
   {
      ...
      if(match)
      {
         ...
         host_node->matched = true;
         /* All items matched! */
            putMorphism(pot, morphism, "mutate_bias_bias2", mutate_bias_host);
