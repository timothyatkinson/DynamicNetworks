...
void applymutate_edge_pickEdge(Morphism *morphism, bool record_changes)
{
   ...
   HostLabel label_e0 = getEdgeLabel(mutate_edge_host, host_edge_index);
   if(record_changes) pushRemarkedEdge(host_edge_index, label_e0.mark);
   changeEdgeMark(mutate_edge_host, host_edge_index, 3);
   ...
   changeNodeMark(mutate_edge_host, host_node_index, 3);
...
static bool fillpot_n1(MorphismPot *pot, Morphism *morphism, Edge *host_edge)
{
   ...
   if(match)
   {
      ...
      host_node->matched = true;
      /* All items matched! */
         putMorphism(pot, morphism, "mutate_edge_pickEdge", mutate_edge_host);
