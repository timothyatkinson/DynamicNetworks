...
bool mutate_param_b1 = true;

static bool evaluateCondition(void)
{
   return (mutate_param_b1);
}

static void evaluatePredicatemutate_param_1(Morphism *morphism)
{
   ...
   if(var_1 + 2 <= 1000) mutate_param_b1 = true;
   else mutate_param_b1 = false;
}
...
void applymutate_param_param2(Morphism *morphism, bool record_changes)
{
...
bool fillpotmutate_param_param2(MorphismPot *pot, Morphism *morphism)
{
   if(1 > mutate_param_host->number_of_nodes || 0 > mutate_param_host->number_of_edges) return false;
...
static bool fillpot_n0(MorphismPot *pot, Morphism *morphism)
{
   ...
   for(nodes = getRootNodeList(mutate_param_host); nodes != NULL; nodes = nodes->next)
   {
      ...
      if(match)
      {
         ...
         host_node->matched = true;
         /* All items matched! */
            putMorphism(pot, morphism, "mutate_param_param2", mutate_param_host);
