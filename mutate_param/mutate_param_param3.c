...
bool mutate_param_b2 = true;
bool mutate_param_b3 = true;
bool mutate_param_b4 = true;

static bool evaluateCondition(void)
{
   return ((mutate_param_b2 && mutate_param_b3) || mutate_param_b4);
}

static void evaluatePredicatemutate_param_2(Morphism *morphism)
{
   ...
   if(var_1 - 1 >= -1000) mutate_param_b2 = true;
   else mutate_param_b2 = false;
}

static void evaluatePredicatemutate_param_3(Morphism *morphism)
{
   ...
   if(!equalHostLists(array0, array1, list_length0, list_length1)) mutate_param_b3 = true;
   else mutate_param_b3 = false;
}

static void evaluatePredicatemutate_param_4(Morphism *morphism)
{
   Assignment assignment_1 = getAssignment(morphism, 1);
   /* If the variable is not yet assigned, return. */
   if(assignment_1.type == 'n') return;
   ...
   if(var_1 - 1 >= 0) mutate_param_b4 = true;
   else mutate_param_b4 = false;
}

static bool match_n0(Morphism *morphism);

bool matchmutate_param_param3(Morphism *morphism)
{
   ...
   for(nodes = getRootNodeList(mutate_param_host); nodes != NULL; nodes = nodes->next)
   {
      ...
      do
      {
         ...
         if(result >= 0)
         {
            new_assignments += result;
            /* Update global booleans for the variable's predicates. */
            evaluatePredicatemutate_param_2(morphism);
            evaluatePredicatemutate_param_4(morphism);
            if(!evaluateCondition())
            {
               /* Reset the boolean variables in the predicates of this variable. */
               mutate_param_b2 = true;
               mutate_param_b4 = true;
               break;
            }
         }
         else break;
...
void applymutate_param_param3(Morphism *morphism, bool record_changes)
{
   string var_0 = getStringValue(morphism, 0);
   int var_1 = getIntegerValue(morphism, 1);
...
bool fillpotmutate_param_param3(MorphismPot *pot, Morphism *morphism)
{
   if(1 > mutate_param_host->number_of_nodes || 0 > mutate_param_host->number_of_edges) return false;
...
static bool fillpot_n0(MorphismPot *pot, Morphism *morphism)
{
   ...
   for(nodes = getRootNodeList(mutate_param_host); nodes != NULL; nodes = nodes->next)
   {
      ...
      if(match)
      {
         ...
         host_node->matched = true;
         /* All items matched! */
            putMorphism(pot, morphism, "mutate_param_param3", mutate_param_host);
