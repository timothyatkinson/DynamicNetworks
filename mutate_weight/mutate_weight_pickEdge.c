...
void applymutate_weight_pickEdge(Morphism *morphism, bool record_changes)
{
   ...
   changeEdgeMark(mutate_weight_host, host_edge_index, 3);

   int host_node_index = lookupNode(morphism, 0);
   Node *node0 = getNode(mutate_weight_host, host_node_index);
   if(!node0->root)
   {
   ...
   Node *node1 = getNode(mutate_weight_host, host_node_index);
   if(!node1->root)
   {
      if(record_changes) pushChangedRootNode(host_node_index);
      changeRoot(mutate_weight_host, host_node_index);
...
static bool fillpot_n1(MorphismPot *pot, Morphism *morphism, Edge *host_edge)
{
   ...
   if(match)
   {
      ...
      host_node->matched = true;
      /* All items matched! */
         putMorphism(pot, morphism, "mutate_weight_pickEdge", mutate_weight_host);
