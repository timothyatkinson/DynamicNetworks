...
{
   if(2 > mutate_delete_edge_host->number_of_nodes || 1 > mutate_delete_edge_host->number_of_edges) return false;
...
static bool match_e0(Morphism *morphism)
{
   ...
   Node *host_node = getNode(mutate_delete_edge_host, start_index);
   ...
   for(counter = 0; counter < host_node->out_edges.size + 2; counter++)
   {
      ...
      {
         Node *end_node = getNode(mutate_delete_edge_host, host_edge->target);
...
static bool match_n1(Morphism *morphism, Edge *host_edge)
{
   Node *host_node = getTarget(mutate_delete_edge_host, host_edge);
...
static bool fillpot_n1(MorphismPot *pot, Morphism *morphism, Edge *host_edge)
{
   ...
   if(match)
   {
      ...
      host_node->matched = true;
      /* All items matched! */
         putMorphism(pot, morphism, "mutate_delete_edge_deleteEdge", mutate_delete_edge_host);
