...
bool matchmutate_add_edge_add_edge_T(Morphism *morphism)
{
...
void applymutate_add_edge_add_edge_T(Morphism *morphism, bool record_changes)
{
   int host_edge_index;
   int source, target;
...
bool fillpotmutate_add_edge_add_edge_T(MorphismPot *pot, Morphism *morphism)
{
   if(2 > mutate_add_edge_host->number_of_nodes || 0 > mutate_add_edge_host->number_of_edges) return false;
...
static bool fillpot_n1(MorphismPot *pot, Morphism *morphism)
{
   ...
   for(host_index = 0; host_index < mutate_add_edge_host->nodes.size; host_index++)
   {
      ...
      if(match)
      {
         ...
         host_node->matched = true;
         /* All items matched! */
            putMorphism(pot, morphism, "mutate_add_edge_add_edge_T", mutate_add_edge_host);
