...
bool matchrandom_d_graph_pick_edge(Morphism *morphism)
{
...
void applyrandom_d_graph_pick_edge(Morphism *morphism, bool record_changes)
{
   ...
   Node *node1 = getNode(random_d_graph_host, host_node_index);
   if(!node1->root)
   {
      if(record_changes) pushChangedRootNode(host_node_index);
      changeRoot(random_d_graph_host, host_node_index);
...
bool fillpotrandom_d_graph_pick_edge(MorphismPot *pot, Morphism *morphism)
{
   if(2 > random_d_graph_host->number_of_nodes || 1 > random_d_graph_host->number_of_edges) return false;
...
static bool fillpot_n1(MorphismPot *pot, Morphism *morphism, Edge *host_edge)
{
   ...
   if(match)
   {
      ...
      host_node->matched = true;
      /* All items matched! */
         putMorphism(pot, morphism, "random_d_graph_pick_edge", random_d_graph_host);
