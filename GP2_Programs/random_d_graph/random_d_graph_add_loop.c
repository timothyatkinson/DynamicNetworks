...
bool random_d_graph_b1 = false;

static bool evaluateCondition(void)
{
   return (!random_d_graph_b1);
}

static void evaluatePredicaterandom_d_graph_1(Morphism *morphism)
{
   ...
   for(counter = 0; counter < source->out_edges.size + 2; counter++)
   {
      ...
      {
         random_d_graph_b1 = true;
         edge_found = true;
         break;
      }
   }
   if(!edge_found) random_d_graph_b1 = false;
}

static bool match_n0(Morphism *morphism);

bool matchrandom_d_graph_add_loop(Morphism *morphism)
{
...
static bool match_n0(Morphism *morphism)
{
   ...
   for(host_index = 0; host_index < random_d_graph_host->nodes.size; host_index++)
   {
      ...
      if(match)
      {
         ...
         evaluatePredicaterandom_d_graph_1(morphism);
...
void applyrandom_d_graph_add_loop(Morphism *morphism, bool record_changes)
{
   int host_edge_index;
   int source, target;
...
bool fillpotrandom_d_graph_add_loop(MorphismPot *pot, Morphism *morphism)
{
   if(1 > random_d_graph_host->number_of_nodes || 0 > random_d_graph_host->number_of_edges) return false;
...
static bool fillpot_n0(MorphismPot *pot, Morphism *morphism)
{
   ...
   for(host_index = 0; host_index < random_d_graph_host->nodes.size; host_index++)
   {
      ...
      if(match)
      {
         ...
         {
            /* All items matched! */
            putMorphism(pot, morphism, "random_d_graph_add_loop", random_d_graph_host);
            /* Reset the boolean variables in the predicates of this node. */
            random_d_graph_b1 = false;
            removeNodeMap(morphism, 0);
            host_node->matched = false;
