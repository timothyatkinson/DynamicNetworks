...
bool matchrandom_d_graph_keep_edge(Morphism *morphism)
{
...
void applyrandom_d_graph_keep_edge(Morphism *morphism, bool record_changes)
{
   ...
   changeEdgeMark(random_d_graph_host, host_edge_index, 0);
...
bool fillpotrandom_d_graph_keep_edge(MorphismPot *pot, Morphism *morphism)
{
   if(2 > random_d_graph_host->number_of_nodes || 1 > random_d_graph_host->number_of_edges) return false;
   int oldPotsize = potSize(pot);
   morphism->weight = 0.100000;
...
static bool fillpot_n1(MorphismPot *pot, Morphism *morphism, Edge *host_edge)
{
   ...
   if(match)
   {
      ...
      host_node->matched = true;
      /* All items matched! */
         putMorphism(pot, morphism, "random_d_graph_keep_edge", random_d_graph_host);
